...
- [x] WF-009 Move `solver/solver.go` to `wmr/solver.go` to reduce top-level package clutter, while preserving the `SolverEngine` interface boundary and current behavior/tests. (commit: pending)
